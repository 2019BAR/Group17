...
```{r}
summary(CPS)
#metro area code,married,edu,industry,employee
```
...
```{r}
tapply(is.na(CPS$MetroAreaCode), CPS$State, mean) %>% sort(decreasing = T)
#montana
```
...
```{r}
MetroAreaMap<-read.csv("data/CountryCodes.csv")
CountryMap<-read.csv("data/MetroAreaCodes.csv")
str(CountryMap)
nrow(CountryMap)
#271
```
...
#when u use merge u will increse metroarea to this variable 
```
...
```{r}
##sum(c(TRUE, FALSE, TRUE, TRUE)) is 3
